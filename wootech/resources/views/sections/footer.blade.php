<div class="bg-blue-200 md:border-b-[8rem] border-gray-400 @if(!is_front_page()) hidden @endif">
...
@if(!empty(get_field('partner_logos', 'option')))
	<section class="container pt-10 pb-6 md:pt-14 md:pb-8">
		@if(!empty(get_field('partner_logos_heading', 'option')))
			<h2 class="text-center text-gray-100" data-aos="fade-up">{!! get_field('partner_logos_heading', 'option') !!}</h2>
		@endif
		<div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5 py-8">
			@foreach(get_field('partner_logos', 'option') as $partner_logo)
				<div class="flex items-center justify-center lg:justify-start p-6 lg:pl-0 grayscale hover:grayscale-0" data-aos="fade-up" data-aos-duration="1000">
					<img src="{!! $partner_logo['url'] !!}" width="auto" height="auto" alt="{!! $partner_logo['title'] !!}">
				</div>
			@endforeach
		</div>
	</section>
...
<footer class="footer bg-blue-100 bg-halftone text-white">
	...
	<div class="border-t border-white border-opacity-25">
		...
		<p class="text-[10px] text-white text-opacity-75 mb-4 md:mb-0">
			© {!! date('Y') !!} <strong>{!! $siteName !!}</strong> All rights reserved <span class="px-1">|</span> <a class="text-white hover:underline" href="#">Privacy Policy</a> <span class="px-1">|</span> <a class="text-white hover:underline" href="#">Cookies</a>
		</p>
		<div class="flex items-center justify-center sm:justify-end">
				<div class="flex-shrink-0">
					<p class="text-[9px] mr-3 text-opacity-80">Developed by</p>
				</div>
				<a href="https://hellenictechnologies.com/" target="_blank"><img src="@asset('images/logo_hellenic-technologies.svg')" width="114" height="53" alt="Hellenic Technologies"></a>
			</div>
		</div>
	</div>
</footer><!--/.footer -->