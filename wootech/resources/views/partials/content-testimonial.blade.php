    ...
    @php
        ...
        if($testimonials_query->have_posts()) {
            while ( $testimonials_query->have_posts() ) : $testimonials_query->the_post();
                ...
                $fimage = get_template_directory_uri().'/assets/images/placeholder.jpg';
        ...
        <div class="js-carousel flex-carousel slick-arrows-mt-[-45px]" data-slick='{
            ...
            @foreach($testimonials_array as $testimonial)
                <div class="js-slide relative flex flex-col bg-white px-6 py-8 sm:px-8 md:p-4 pb-2 lg:pt-16 lg:px-12 lg:pb-1 transition duration-300 hover:shadow-xs mb-5 lg:mb-8">
                    <div class="font-light mb-8">{!! $testimonial['content'] !!}</div>
                    <div class="flex items-center mt-auto mb-6">
                        <img class="rounded-full flex-shrink-0 mr-4" src="{!! $testimonial['image'] !!}" width="60" height="60" alt="{!! $testimonial['title'] !!}">
                        @if(!empty($testimonial['title']))
                            <span class="text-gray-300 font-light text-[13px]">{!! $testimonial['title'] !!}@if($testimonial['location']), {!! $testimonial['location'] !!} @endif</span>
                        @endif
                    </div>
                    <div class="flex items-center justify-between border-t border-gray-400 py-5 min-h-[80px]">
                        @if(!empty($testimonial['video_url']))
                            <a class="text-red-50 text-[13px] font-bold flex flex-shrink-0 items-center hover:text-red-500" href="{!! $testimonial['video_url'] !!}" target="_blank">
